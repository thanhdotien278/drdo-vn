...
If `{story_key}` is not empty and `/Users/Super/drdovn/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `/Users/Super/drdovn/_bmad/render/bmad-build/drdovn-f29c8d805094/985d2fb5812ad2b760af/sync-sprint-status.md` with `{target_status}` = `in-progress`.
...
**When to stop and replan.** Stop coding if the request left out something the user would notice in the result. Write the gap in `## Implementation Notes`. Then update `{spec_file}`: add back `## Code Map` (filled in from what you learned while implementing) and `## Open Questions` (one question per gap), set `route: 'full'` and `status: 'draft'`. Go back to `/Users/Super/drdovn/_bmad/render/bmad-build/drdovn-f29c8d805094/985d2fb5812ad2b760af/step-02-plan.md` step 6.
...
If a lens needs subagents and you cannot launch them, write the full prompt for each lens under `/Users/Super/drdovn/_bmad-output/implementation-artifacts` (with placeholders filled in, not just file paths). Stop and ask the user to run each prompt in a separate session and paste back the findings.
...
- **defer** — Everything else: old bugs not caused by this change, ideas for later, groups where every member is `maybe-false` and would be `medium` or `high` if true (record that severity marked unverified, and what would prove it; if it would only be `low`, reject it), or fixes that would edit CLAUDE.md, AGENTS.md, rules, or specs. Add one entry to `/Users/Super/drdovn/_bmad-output/implementation-artifacts/deferred-work.md`:
...
If `{story_key}` is not empty and `/Users/Super/drdovn/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `/Users/Super/drdovn/_bmad/render/bmad-build/drdovn-f29c8d805094/985d2fb5812ad2b760af/sync-sprint-status.md` with `{target_status}` = `review`.
